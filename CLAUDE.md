...
**BRANCH `eval/uncoached-prompt-reporting` (awaiting Jaivir review before any full-matrix run):**

- **Uncoached prompt is now the default** (src/nostart/prompts.py): role + tool reference + cost one-liner + finish() format ONLY. The old prompt (which dictated measure-first / isolate-repair-verify and enumerated grader rules) is preserved verbatim as `--prompt-variant coached`, recorded in every results row and transcript header.
