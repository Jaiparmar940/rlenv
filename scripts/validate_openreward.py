...
def ors_model_episode(env, task, model_id: str) -> float:
    """Minimal agent loop over the ORS HTTP interface (Anthropic API)."""
    import anthropic
    ...
    with env.session(task=task) as session:
        ...
        while len(messages) < MESSAGE_LIMIT:
            ...
            if texts:
                last_text = texts[-1]
            if not tool_uses:
                break  # plain assistant message ends the rollout
            messages.append(
            ...
            for call in tool_uses:
                ...
                if hasattr(output, "blocks"):
                    ...
                    payload = f"Error: {getattr(output, 'error', output)}"
                results.append(
                    {"type": "tool_result", "tool_use_id": call.id,
                     "content": payload}
                )
            messages.append({"role": "user", "content": results})
        # Harness-side cutoff: grade the last plain message as the diagnosis
        # (same fallback the Inspect scorer applies to output.completion).
        result = session.call_tool("grade_final_message", {"message": last_text})
        output = getattr(result, "output", result)
...
def part_b(env, model: str) -> None:
    ...
    for scenario_id, task_id in SCENARIO_TASKS.items():
        ors = ors_model_episode(env, tasks[task_id], sdk_model)
