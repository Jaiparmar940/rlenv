...
**State machine:** `attempt_start` → `starts` sets the engine running. It stays running until something implies shutting it off: `replace_part` (wrenching on the car) or a `measure_voltage`/`read_pid` in any non-running key state. Requesting `running` while the engine is not running RAISES. `read_pid` takes an explicit `engine_state` like `measure_voltage` — valid: `key_on`, `cranking`, `running` (the ECU is asleep at `key_off`, so that RAISES); every payload names the `engine_state` it was read in, so an engine-off `alt_output_v` (rail voltage) is visibly not a charging measurement. `rpm` is a live tach: 0 at `key_on`, crank speed while `cranking` (95 slow / 180 normal, 0 when the starter can't spin), ≈ 700 idle while `running`.
